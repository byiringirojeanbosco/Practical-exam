      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <Badge tone={queue.completed ? "green" : "blue"}>
            {queue.completed ? t("hospital.ready") : t("hospital.liveQueue")}
          </Badge>
          <h3 className="mt-3 text-xl font-bold text-gray-900">
            {queue.completed
              ? t("hospital.readyText")
              : t("hospital.ahead", { count: queue.position })}
          </h3>
        </div>
        <div className="rounded-lg bg-[var(--color-primary-soft)] px-4 py-3 text-center">
          <p className="text-2xl font-bold text-[var(--color-primary-text)]">
            {queue.estimateMinutes}
          </p>
          <p className="text-xs font-semibold text-[var(--color-primary-text)]">min</p>
        </div>
      </div>
...
export default function HospitalPage() {
  ...
  const { state, checkIn, advanceQueue, completeVisit, t } = useAppointment();
  ...
  return (
    <div className="space-y-5">
      <section className="space-y-2">
        <Badge tone="blue">{t("hospital.step")}</Badge>
        <h2 className="text-2xl font-bold text-gray-900">{t("hospital.title")}</h2>
        <p className="text-sm leading-6 text-gray-600">{t("hospital.subtitle")}</p>
      </section>
      ...
      <Card>
        <div className="flex items-start justify-between gap-3">
          ...
          <Badge tone={checkInState.status === "not_started" ? "gray" : "green"}>
            {checkInState.status === "not_started" ? t("hospital.notChecked") : t("hospital.checked")}
          </Badge>
        </div>
      </Card>

      <Card>
        <h3 className="text-base font-bold text-gray-900">{t("hospital.arrival")}</h3>
        <ol className="mt-4 space-y-3">
          {hospitalDirections.map((direction, index) => (
            <li key={direction} className="flex gap-3">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--color-primary-soft)] text-sm font-bold text-[var(--color-primary-text)]">
                {index + 1}
              </span>
      ...
      {checkInState.status === "not_started" ? (
        ...
        <QueuePanel queue={queue} t={t} />
      )}

      {queue.completed ? (
        <Card className="border-green-200 bg-green-50">
          <h3 className="text-lg font-bold text-green-900">{t("hospital.doctorReady")}</h3>
          <p className="mt-2 text-sm leading-6 text-green-800">{t("hospital.afterVisit")}</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <Button variant="secondary" onClick={completeVisit}>
              {t("hospital.complete")}
            </Button>
            <Button onClick={() => navigate("/followup")}>{t("hospital.viewCare")}</Button>
          </div>
        </Card>
      ) : null}
    </div>
