...
export default function DoctorDiscoveryPage() {
  ...
  const { state, selectDoctor, t } = useAppointment();
  ...
  return (
    <div className="space-y-5">
      <section className="space-y-2">
        <Badge tone="blue">{t("doctors.step")}</Badge>
        <h2 className="text-2xl font-bold text-gray-900">{t("doctors.title")}</h2>
        <p className="text-sm leading-6 text-gray-600">{t("doctors.subtitle")}</p>
      </section>

      <Card>
        <h3 className="text-sm font-bold text-gray-900">{t("doctors.selectedSymptoms")}</h3>
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {index === 0 ? <Badge tone="green">{t("doctors.bestMatch")}</Badge> : null}
                  </div>
